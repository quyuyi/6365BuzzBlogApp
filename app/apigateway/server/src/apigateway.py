...
def retrieve_recommended_posts(keyword):
  try:
    search_size = int(flask.request.args["search_size"])
    return_size = int(flask.request.args["return_size"])
  except KeyError:
    return ({}, 400)
  print("get parameters search_size {}, return_size {}".format(search_size, return_size))
  with thrift_client_factory.get_recommendation_client() as recommendation_client:
    print("1")
    rec_posts = recommendation_client.retrieve_recommended_posts(keyword, search_size, return_size)
    print("2")
  return flask.jsonify([{
